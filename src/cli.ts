...
import { IGNORE_FILES, renderIgnorePatch } from "./report/ignore.js";
...
Usage:
  ...
  sherlock --emit-ignore [path]  print .claudeignore/.cursorignore patches for safe ignore suggestions
...
Options:
  ...
  --emit-ignore   unified diffs for .claudeignore and .cursorignore; never writes to the repo
  ...
  try {
    ...
    if (args.json) {
      ...
      for (const ignoreFile of IGNORE_FILES) {
        ...
        try {
          existing = await fs.readFile(path.join(result.root, ignoreFile), "utf8");
        } catch (err) {
          if ((err as NodeJS.ErrnoException).code !== "ENOENT") throw err;
        }
        const patch = renderIgnorePatch(existing, result.findings, ignoreFile);
